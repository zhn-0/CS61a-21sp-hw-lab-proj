...
def num_splits(s, d):
    ...
    def func(a, b, i):
        if i == sz:
            return int(abs(a - b) <= d)
        return func(a, b + s[i], i + 1) + func(a + s[i], b, i + 1)
    return func(0, 0, 0) // 2
...
class Account:
    ...
    def __repr__(self):
        "*** YOUR CODE HERE ***"
        return f'Accountholder: {self.holder}, Deposits: {self.d}, Withdraws: {self.w}'
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            a, b = helper_align(skeleton_idx + 1, code_idx + 1)
            possibilities.append((skel_char + a, b))
        # Insert
        a, b = helper_align(skeleton_idx, code_idx + 1)
        possibilities.append((f'+[{code_char}]' + a, b + 1))
        # Delete
        a, b = helper_align(skeleton_idx + 1, code_idx)
        possibilities.append((f"-[{skel_char}]" + a, b + 1))
...
def foldr(link, fn, z):
    ...
    def func(link):
        if link is Link.empty:
            return z
        return fn(link.first, func(link.rest))
    return func(link)
