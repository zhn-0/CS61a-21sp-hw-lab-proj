    ...
    def __repr__(self):
        ...
        for t in self.transactions:
            if t[0] == 'deposit':
                ...
                b += 1
        return f'Accountholder: {self.holder}, Deposits: {a}, Withdraws: {b}'
...
def insert(link, value, index):
    ...
    if index == 0:
        while link.rest is not Link.empty:
            link.first, value = value, link.first
            link = link.rest
        link.first, link.rest = value, Link(link.first)
        return
    i = 0
    while i + 1 < index:
        link = link.rest
...
def deep_len(lnk):
    ...
    if not isinstance(lnk, Link):
        return 0
    elif lnk.rest is None and not isinstance(lnk.first, Link):
        return 1
    else:
...
def long_paths(t, n):
    ...
    def func(t, tmp):
        if len(t.branches) == 0:
            if len(tmp) >= n:
                ret.append(tmp + [t.label])
            return
        for nxt in t.branches:
            func(nxt, tmp + [t.label])
    func(t, [])
    return ret
