...
def berry_finder(t):
    ...
    return label(t) == 'berry' or bool(sum(berry_finder(i) for i in branches(t)))
...
def add_trees(t1, t2):
    ...
    if len(b1) != len(b2):
        if len(b1) > len(b2):
            b2 = b2 + [tree(0) for i in range(len(b1) - len(b2))]
        else:
            b1 = b1 + [tree(0) for i in range(len(b2) - len(b1))]
    return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in zip(b1, b2)])
