...
'''
简记：
optional 2 很难，虽然过了测试但我估计会有bug
简单来说就是做一个函数链表，但py写起来有点困难...
'''
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if self.is_container() is True and place.ant.is_container() is False:
                self.contain_ant(place.ant)
                place.ant = self
            elif self.is_container() is False and place.ant.is_container() is True:
    ...
    def buff(self):
        ...
        if self.buffed == False:
            self.damage *= 2
            self.buffed = True
        if self.is_container() and self.contained_ant is not None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        p, cnt = self.place, 0
        while cnt < self.min_range and p is not None:
            p = p.entrance
            cnt += 1
        if p is None:
            return None
        while cnt <= self.max_range and p.is_hive() is False:
            if len(p.bees) > 0:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        return self.contained_ant is None and is_container(other) is False
        # END Problem 8
...
# END Problem EC
    ...
    def __init__(self, health=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(health)
        self.identity = QueenAnt.trueQueen
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_health(amount)
        if self.identity is True and self.health <= 0:
            bees_win()
        # END Problem EC
...
class Bee(Insect):
    ...
    def slow(self, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        def func(gamestate, f=None):
            if gamestate.time % 2 == 0:
                f(gamestate)
        self.apply_status(func, self.action, length)
        # END Problem Optional 2
    ...
    def apply_status(self, status, previous_action, length):
        ...
        "*** YOUR CODE HERE ***"
        def func(gamestate):
            nonlocal length, status
            print('DEBUG', self.name, length)
            if length > 0:
                length -= 1
                status(gamestate, previous_action)
            else:
                status = previous_action
                self.advancing = True
                status(gamestate)
        self.action = func
        # END Problem Optional 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem Optional 3
        ret, p, distance = {}, self.place, 0
        while not p.is_hive() and p is not None:
            if p.ant is not None:
                if p.ant is not self:
                    ret[p.ant] = distance
                if p.ant.is_container() and p.ant.contained_ant is not self:
                    ret[p.ant.contained_ant] = distance
            for bee in p.bees:
                ret[bee] = distance
            p = p.entrance
            distance += 1
