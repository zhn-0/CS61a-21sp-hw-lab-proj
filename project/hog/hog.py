...
def more_boar_cal(score):
    """
    Return the leftmost digit of score
         and the second leftmost digit of score
    """
    if score < 10:
        return [0, score]
    ret=0
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_score_a, player_score_b = more_boar_cal(player_score)
    opponent_score_a, opponent_score_b = more_boar_cal(opponent_score)
    return player_score_a < opponent_score_a and player_score_b < opponent_score_b
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        score = take_turn(num_rolls, opponent_score, dice, goal)
        if who == 0:
            score0 += score
            not_change_player = more_boar(score0, score1)
        else:
            score1 += score
            not_change_player = more_boar(score1, score0)
        if not_change_player is False:
            who = next_player(who)
    ...
    def say(score0, score1):
        ...
        if score > last_score:
            if score - last_score > running_high:
                r_h = score - last_score
                print("Player", who, "has reached a new maximum point gain.",
                        r_h, "point(s)!")
        return announce_highest(who, score, r_h)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def f(*args):
        ...
        for i in range(trials_count):
            ret_sum += original_function(*args)
        return ret_sum / trials_count
    return f
    # END PROBLEM 8
