...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    tmp = [p for p in paragraphs if select(p)]
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def func(ss):
        for s in ss.lower().split():
            a = "".join(c for c in s if c >= 'a' and c <= 'z')
            if a in topic:
                return True
        return False
    return func
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    d = [diff_function(typed_word, v, limit) for v in valid_words]
    if min(d) > limit:
        return typed_word
    return valid_words[d.index(min(d))]
    # END PROBLEM 5
...
def sphinx_switches(start, goal, limit):
    ...
    # BEGIN PROBLEM 6
    def func(idx, cnt):
        if idx >= min(len(start), len(goal)):
            return cnt + abs(len(goal) - len(start))
        if cnt > limit:
            return limit + 1
        return func(idx + 1, cnt + (1 if start[idx] != goal[idx] else 0))
    return func(0, 0)
    # END PROBLEM 6
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    return game(words, [[p[i+1] - p[i] for i in range(len(p) - 1)] for p in times_per_player])
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for i in word_indices:
        d = [t[i] for t in all_times(game)]
        ret[d.index(min(d))].append(all_words(game)[i])
    return ret
    # END PROBLEM 10
