...
def store_digits(n):
    ...
    def func(idx):
        if idx == 0:
            return Link(digit[idx])
        return Link(digit[idx], func(idx - 1))
    return func(len(digit) - 1)
...
def path_yielder(t, value):
    ...
    for nxt in t.branches:
        for l in path_yielder(nxt, value):
            "*** YOUR CODE HERE ***"
            yield [t.label] + l
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            return t.label
        return min([t.label] + [bst_min(nxt) for nxt in t.branches])
    def bst_max(t):
        if t.is_leaf():
            return t.label
        return max([t.label] + [bst_max(nxt) for nxt in t.branches])
    if t.is_leaf():
        return True
    if len(t.branches) == 1:
...
def generate_preorder(t):
    ...
    if t.is_leaf():
        return
    for nxt in t.branches:
        for r in list(generate_preorder(nxt)):
            yield r
